...
class MirroredCircuit(Circuit):
    def __init__(self, R, Z, current=0.0, turns=1, control=True, area=AreaCurrentLimit(), symmetric=True):
        coils = [
            (
                "U",
        ...
        super().__init__(coils, current, control)
        self._R = R
        self._Z = Z
...
def MirroredCoil(
    ...
    """
    return MirroredCircuit(R, Z, current, turns, control, area, symmetric)
...
class PoloidalFieldSensor(Sensor):
    ...
    def get_measure(self, tokamak, eq):
        ...
        if self.status:
            ...
            field = (tokamak.Br(self.R, self.Z)) * np.cos(self.theta) + (tokamak.Bz(self.R, self.Z)) * np.sin(
                self.theta)

            if eq is not None:
                field += (eq.plasmaBr(self.R, self.Z)) * np.cos(self.theta) + (eq.plasmaBz(self.R, self.Z)) * np.sin(
                    self.theta)
